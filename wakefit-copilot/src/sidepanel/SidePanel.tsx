...
export function SidePanel() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      text: "Hi! I'm your Wakefit Copilot. Ask me anything about specs, dimensions, or trial terms.",
    },
  ])
  ...
  async function handleSend() {
    ...
    try {
      ...
      const response = await chrome.tabs.sendMessage(tab.id!, { type: 'QUERY', payload: text })
  ...
  return (
    <div class="flex h-screen flex-col bg-gray-50">
      <header class="flex items-center gap-2 bg-wakefit-primary px-4 py-3 text-white shadow">
        ...
        <div>
          <p class="text-sm font-semibold">Wakefit Copilot</p>
          ...
          <div key={i} class={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              class={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed shadow-sm ${
                msg.role === 'user'
                  ? 'rounded-br-sm bg-wakefit-primary text-white'
      ...
      <div class="flex gap-2 border-t border-gray-200 bg-white p-3">
        <input
          value={input}
        ...
        <button
          onClick={handleSend}
          disabled={loading}
          class="rounded-full bg-wakefit-primary px-4 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90 disabled:opacity-50"
        >
          Send
        </button>
      </div>
